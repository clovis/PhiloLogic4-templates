...
function toggle_frequency() {
    ...
    var spinner = '<img src="/philo4beta2/${dbname}/js/spinner-round.gif" alt="Loading..." />';
    if ($("#toggle_frequency").hasClass('show_frequency')) {
        ...
        $.getJSON("/philo4beta2/${dbname}/scripts/get_frequency.py?frequency_field=" + field + '&${q['q_string']}', function(data) {
            $(".loading").hide().empty();
